sap.ui.define([
	...
	function getFrameUrl(sHash, sUrlParameters) {
		var sUrl = jQuery.sap.getResourcePath("be/ehb/mobile/EnterpriseFlexso/app", ".html");
		sHash = sHash || "";
		sUrlParameters = sUrlParameters ? "?" + sUrlParameters : "";

		if (sHash) {
			sHash = "#EnterpriseFlexso-display&/" + (sHash.indexOf("/") === 0 ? sHash.substring(1) : sHash);
		} else {
			sHash = "#EnterpriseFlexso-display";
		}

		return sUrl + sUrlParameters + sHash;
	}

	return Opa5.extend("be.ehb.mobile.EnterpriseFlexso.test.integration.pages.Common", {
		...
		getMockServer: function () {
			return new Promise(function (success) {
				Opa5.getWindow().sap.ui.require(["be/ehb/mobile/EnterpriseFlexso/localService/mockserver"], function (mockserver) {
